...
function App() {
  ...
  const updateQuantityComposite = (compositeId: string, delta: number) => {
    setCart(prev => prev.map(item => {
      const key = `${item.id}-${item.selectedVariant.weight}${item.selectedIntensity ? `-${item.selectedIntensity}` : ''}${item.selectedRoast ? `-${item.selectedRoast}` : ''}`;
      if (key === compositeId) {
  ...
  const Footer = () => (
    <footer className="bg-coffee-950 text-coffee-200 py-12 mt-auto">
      <div className="container mx-auto px-4 text-center">
        ...
        <div className="flex justify-center gap-6 text-sm uppercase tracking-widest opacity-60 items-center mb-4 mt-8 flex-wrap">
          ...
          <a href="https://wa.me/919985802734" target="_blank" rel="noopener noreferrer" className="hover:text-gold-400 transition-colors">WhatsApp</a>
          <a href="tel:+919985802734" className="hover:text-gold-400 transition-colors flex items-center gap-1 font-bold">
        ...
        <div className="bg-coffee-50 min-h-screen flex flex-col animate-fade-in">
          <div className="relative h-[400px] flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0 bg-coffee-900">
              <img src="/assets/hero_bg.jpg" alt="Shop Hero" className="w-full h-full object-cover opacity-40" />
    ...
    return (
      <div className="bg-coffee-50 min-h-screen flex flex-col">
        ...
        <ProductList 
          ...
          onProductClick={(p) => { setSelectedProduct(p); setIsModalOpen(true); }} 
          onAddToCart={(p) => { setSelectedProduct(p); setIsModalOpen(true); }} 
          onNavigateToShop={() => setView(AppView.SHOP)}
          onUpdateQuantity={updateQuantityComposite}
          onQuickAdd={(p, v) => addToCart({ ...p, selectedVariant: v, quantity: 1 })}
        />
        <ProducerSection />
  ...
  return (
    <>
      ...
      <CartSidebar isOpen={isCartOpen} onClose={() => setIsCartOpen(false)} items={cart.map(c => ({ ...c, id: `${c.id}-${c.selectedVariant.weight}${c.selectedIntensity ? `-${c.selectedIntensity}` : ''}${c.selectedRoast ? `-${c.selectedRoast}` : ''}` }))} onUpdateQuantity={updateQuantityComposite} onRemoveItem={removeItemComposite} onCheckout={() => { setIsCartOpen(false); if (currentUser) { setView(AppView.CHECKOUT); } else { setView(AppView.AUTH); } }} />
    </>
  );
}
